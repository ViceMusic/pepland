...
CONFIG = {
    ...
    # Repeated runs
    # ============================================================
    # Only run once for error-check JSON export.
    "num_runs": 1,
    "base_seed": 0,
...
def export_test_check_json(model, loader, test_dataset, device):
    """
    Export test predictions in the original test dataset order.

    Output format:
    [
        {"SMILES": "...", "pred": 0/1, "true": 0/1},
        ...
    ]

    Note:
    test_loader must use shuffle=False. The current make_test_loader()
    already uses shuffle=False, so the output follows the original test
    CSV order after dropna/reset_index in SupervisedMolGraphDataset.
    """
    model.eval()
    all_probs = []
    all_labels = []
    ...
    for i in range(len(test_dataset.df)):
        records.append({
            "SMILES": str(test_dataset.df.iloc[i][smiles_col]),
            "pred": int(y_pred[i]),
            "true": int(y_true[i]),
        })

    os.makedirs(CONFIG["output_dir"], exist_ok=True)
    out_path = os.path.join(CONFIG["output_dir"], f"check_{task}.json")

    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(records, f, ensure_ascii=False, indent=2)

    print(f"[Check JSON] saved to {out_path}")
    return out_path
...
def run_once(run_id):
    ...
    # Export test prediction JSON once.
    # Since test_loader uses shuffle=False, records follow the original test CSV order.
    export_test_check_json(model, test_loader, test_dataset, device)
